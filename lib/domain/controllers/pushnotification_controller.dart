...
class PushNotifications {
  ...
  Future<void> init() async {
    try {
      ...
      if (!isEnabled) {
        debugPrint('Push notifications are disabled in settings');
        return;
      }
  ...
  Future<void> _handleForegroundMessage(RemoteMessage message) async {
    ...
    if (!isEnabled) {
      debugPrint('Notifications disabled, not showing foreground message');
      return;
    }
  ...
  Future<void> _showLocalNotification({
    ...
    // Double check if notifications are enabled
    final isEnabled = await isNotificationsEnabled();
    if (!isEnabled) {
      debugPrint('Notifications disabled, not showing local notification');
      return;
    }
