  ...
  Future<ApiResponse<String>> sendotp({required String mobilenumber}) async {
    try {
      ...
      Response response = await dio
          .post(Endpoints.login, data: {'occupantMobile': mobilenumber}
             
              );

      final responseData = response.data;
      if (!responseData["error"] && responseData["status"] == 200) {
         final customerId = responseData["data"]["flatId"].toString();
...
// ///////////// verifyotp/////////////
  Future<ApiResponse> verifyotp(
      {required String otp, required String flatId}) async {
    try {
...
//   /////////////resendotp/////////////
  Future<ApiResponse> resendotp({required String flatId}) async {
    try {
      // final token = await getUserToken();
      Response response =
          await dio.post(Endpoints.resendOTP, data: {"flatId":flatId});
  ...
  ///////////////////////registernewdivision/////////////////////
    Future<ApiResponse> registernewDivision({required RegisterNewdivisionModel division}) async {
    try {
      // final token = await getUserToken();
      Response response =
          await dio.post(Endpoints.registerdivison, data:division);
